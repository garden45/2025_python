...
pygame.display.set_caption("똥피하고 사과, 당근먹기")
...
carrot_img = pygame.image.load("C:/2025_python/game/carrot.png")
carrot_img = pygame.transform.scale(carrot_img, (40, 40))
...
# 여러 개 사과를 관리하기 위한 리스트 (Rect, 속도)
apples = []   # 각 원소: {"rect": Rect, "vx": int, "vy": int},
              # rext:위치정보, vx:x축 이동, vy:Y축 이동
carrots = []

# 사과 생성 주기 관리용 카운터
apple_spawn_timer = 0
APPLE_SPAWN_INTERVAL = 30  # 프레임마다 체크 (약 0.5초 간격 @60FPS 기준)
carrot_spawn_timer = 30
CARROT_SPAWN_INTERVAL = 45

score = 0
running = True
game_over = False 

def spawn_item(item_list):
    # 화면 네 가장자리(위/아래/좌/우)에서 사과 하나 생성
    ...
    item_list.append({"rect": rect, "vx": vx, "vy": vy}) # 리스트에 아이템 추가
...
while running:
    ...
    if not game_over:
        ...
        # 일정 주기마다 새로운 사과, 당근 생성(사방팔방에서 계속 나옴)
        apple_spawn_timer += 1
        ...
        # 각 사과 이동 & 충돌 처리
        def update_items(item_list, item_name):
            global score
            new_items = []
        ...
        apples = update_items(apples, "사과")
        carrots = update_items(carrots, "당근")
    ...
    for carrot in carrots:
        screen.blit(carrot_img, carrot["rect"])
